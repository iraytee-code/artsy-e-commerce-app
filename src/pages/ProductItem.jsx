...
import SwiperItem from "../utils/swiper/Swiper";
import ProductSubs from "../layouts/Products/ProductSub";

function ProductItem() {
  const { productId } = useParams();
  const { products, setCart, cart, toggleFavorite, favorite } =
    useContext(Context);
        ...
        <div className="max-w-[1064px] mx-auto">
          <section className="font-medium md:leading-[200%] mb-8 md:mb-10 border-b-[0.5px] md:border-b-0 border-b-grey leading-[52px]">
            <span className="text-[#BCB7B7]">
              <Link to="/">Home/</Link>{" "}
              <Link to="marketplace">Marketplace/</Link> Editorial/
            </span>
            <span className="text-black">
              {openProduct && toTitleCase(openProduct.name)}
            </span>
          </section>
          ...
          <main className="md:border flex-col md:flex-row border-grey flex text-grey leading-[97%]">
            <div className="md:w-2/5 shrink-0 md:border-r border-r-grey md:py-5 md:px-3">
              <img
                src={openProduct && openProduct.url}
                alt=""
                className="w-full"
              />
            </div>
            <div className="w-full flex flex-col">
              <div className="flex md:border-b border-b-grey md:px-5 pt-4 md:py-3 justify-between items-center">
                <h2 className="md:text-[2rem] font-medium md:font-bold ">
                  {openProduct && toTitleCase(openProduct.name)}
                </h2>
                ...
                <span className="font-medium md:hidden">
                  ${openProduct && openProduct.price.usd}
                </span>
              </div>
              <div className="md:px-5 py-6 grow-1  ">
                <p className="mb-3 text-lg text-black md:text-[#616161]">
                  Creator :{" "}
                  <span className="text-[#4693ED]">
                    {openProduct && toTitleCase(openProduct.creator)}
                  </span>
                </p>
                <p className="mb-3 md:text-black">
                  {openProduct && toTitleCase(openProduct.origin)}
                </p>
                <p className="mb-4 md:text-black text-lg font-medium">
                  Total views: <span>{openProduct && openProduct.views}</span>
                </p>
                <div className="mb-6 text-2xl md:text-xl font-medium">
                  <button onClick={decreaseQty} className="mr-3">
                    -
                  </button>
                ...
                <div className="flex">
                  <button
                    ...
                    id="add-to-cart"
                    onClick={handleClick}
                    data-id={`${openProduct && openProduct.id}`}
                    className="bg-blue-500 disabled:bg-blue/[0.5] rounded-[3px] px-6 md:px-8 h-[3.5rem] flex items-center text-white font-medium"
                  >
                  ...
                  <button className="border h-[3.5rem] border-grey px-3 ml-4">
                    <svg
                      ...
                      className="h-[25px] w-[27px] md:h-[42px]  text-red-500"
                      width="49"
                      height="34"
                      viewBox="0 0 49 34"
                      fill="none"
                    ...
                    >
                      <path
                        className={
                          favorite.some((item) => item.id == productId)
                            ? "fill-red-500"
                            : "stroke-black"
                        }
              ...
              <div className="px-2 md:px-5 mt-auto py-5 md:py-6 border-y border-y-grey">
                <ProductSubs
                  title="Description"
                  content="Lorem ipsum dolor sit, amet consectetur adipisicing elit. Enim, iste at dignissimos commodi deserunt accusamus similique accusantium voluptates pariatur? Dolorum odit suscipit cupiditate maxime quos odio ipsum dolores porro cumque?"
                />
              </div>
              ...
              <div className="px-2 md:px-5 py-5 md:py-6 border-b border-b-grey md:border-b-0">
                <ProductSubs
                  title="Status"
                  content="Lorem ipsum dolor sit, dolorum odit suscipit cupiditate maxime quos odio ipsum dolores porro cumque?"
                />
              </div>
            </div>
          </main>
          ...
          <h4 className="font-medium text-[1.5rem] text-grey-dark mt-[100px] mb-[30px] md:hidden">
          ...
          <Link to="/marketplace" className="hidden md:block">
            <div className="my-[100px] border-img w-fit p-[2px] rounded-[1rem] overflow-hidden mx-auto">
              <button className=" h-[3.5rem] px-8 text-[26px] rounded-[1rem] font-medium leading-[25px] bg-white">
                <span className="text-gradient">Explore All</span>
              </button>
            </div>
          </Link>
        </div>
