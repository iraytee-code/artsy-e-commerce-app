...
function Auctions() {
  ...
  return (
    <Transitions>
      {showLiveBid.status && (
        <div className="fixed top-0 left-0 w-full h-screen bg-white overflow-scroll z-50">
          <Livebid
            auctionProduct={auctionProduct}
            prodId={showLiveBid.id}
            handleClick={closeLiveBid}
          />
        </div>
      )}
        ...
        <div className="max-w-[1064px] mx-auto  px-4 md:px-0">
          <article className="text-lg leading-[200%] font-medium mb-10">
            <p className="text-[#BCB7B7] hidden md:block">
              <Link to="/">Home</Link>/{" "}
              <span className="text-grey">Auctions</span>
            </p>
            <p className="text-black leading-[157%]">
          ...
          <section className="relative">
            <div
              ref={scrollContainer}
              className="flex gap-8 w-full overflow-x-scroll"
            >
              <div className="absolute top-10 left-0 flex w-full justify-between z-10 md:hidden">
                <button
                  onClick={scrollReverse}
                  className="h-10 w-10 rounded-full backdrop-blur-[4.681px] bg-white/[0.2] flex items-center justify-center shadow-6xl"
                >
                  <svg
                    width="6"
                    height="9"
                  ...
                  >
                    <path
                      d="M2.04693 4.81002L5.26855 1.5884L4.34828 0.668121L0.206373 4.81002L4.34828 8.95193L5.26855 8.03165L2.04693 4.81002Z"
                      fill="white"
                    />
                  </svg>
                </button>
          ...
          <section>
            <h4 className="text-xl text-black leading-[200%] font-bold">
